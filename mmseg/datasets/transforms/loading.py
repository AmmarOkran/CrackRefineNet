...
class LoadAnnotations(MMCV_LoadAnnotations):
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        """
        if not results['has_strong']:
            # img_bytes = fileio.get(
            #     results['seg_map_path'], backend_args=self.backend_args)
            gt_semantic_seg = np.zeros_like(results['img'][:,:,0], dtype=np.uint8)
        else:
            img_bytes = fileio.get(
                results['seg_map_path'], backend_args=self.backend_args)
...
@TRANSFORMS.register_module()
class LoadAnnotationsBoundarires(BaseTransform):
    """Load annotations for semantic segmentation provided by dataset.
    ...
    def _load_boundaries_map(self, results: dict) -> None:
        """Private function to load semantic segmentation annotations.
        ...
        results['gt_bound_map'] = self.Bextraction(results['gt_seg_map'])


    def Bextraction(self, img):
        import cv2
        # img = img.numpy()
    ...
    def transform(self, results: dict) -> dict:
        ...
        self._load_boundaries_map(results)
        return results
...
@TRANSFORMS.register_module()
class LoadStWeakAnnotations(MMCV_LoadAnnotations):
    """Load annotations for semantic segmentation provided by dataset.
    ...
    def _load_stweak_map(self, results: dict) -> None:
        """Private function to load semantic segmentation annotations.
        ...
        Returns:
            dict: The dict contains loaded semantic segmentation annotations.
        """
        if not results['has_weak']:
            # img_bytes = fileio.get(
            #     results['seg_map_path'], backend_args=self.backend_args)
            gt_semantic_stweak = np.zeros_like(results['img'][:,:,0], dtype=np.uint8)
        else:
            img_bytes = fileio.get(
                results['stweak_map_path'], backend_args=self.backend_args)
            gt_semantic_stweak = mmcv.imfrombytes(
                img_bytes, flag='unchanged',
                backend=self.imdecode_backend).squeeze().astype(np.uint8)
        ...
        # if self.reduce_zero_label:
        #     # avoid using underflow conversion
        #     gt_semantic_stweak[gt_semantic_stweak == 0] = 255
        #     gt_semantic_stweak = gt_semantic_stweak - 1
        #     gt_semantic_stweak[gt_semantic_stweak == 254] = 255
        # # modify if custom classes
        # if results.get('label_map', None) is not None:
        #     # Add deep copy to solve bug of repeatedly
        #     # replace `gt_semantic_seg`, which is reported in
        #     # https://github.com/open-mmlab/mmsegmentation/pull/1445/
        #     gt_semantic_stweak_copy = gt_semantic_stweak.copy()
        #     for old_id, new_id in results['label_map'].items():
        #         gt_semantic_stweak[gt_semantic_stweak_copy == old_id] = new_id
        results['gt_stweak_map'] = gt_semantic_stweak
        # results['seg_fields'].append('gt_stweak_map')

    def transform(self, results: dict) -> dict:

        self._load_stweak_map(results)
        return results
...
@TRANSFORMS.register_module()
class LoadHeatAnnotations(MMCV_LoadAnnotations):
    """Load annotations for semantic segmentation provided by dataset.
    ...
    def _load_heat_map(self, results: dict) -> None:
        ...
        Returns:
            dict: The dict contains loaded semantic segmentation annotations.
        """
        # Read 16-bit heatmap and normalize it correctly
        img_bytes = fileio.get(
            results['heat_map_path'], backend_args=self.backend_args)
        # Read as a 16-bit grayscale image (preserving original values)
        gt_semantic_heat = mmcv.imfrombytes(
            img_bytes, flag='unchanged', backend=self.imdecode_backend).astype(np.float32)  # Convert to float32
        # Normalize to [0,1] range (since it's stored as uint16 with max 65535)
        gt_semantic_heat /= 255.0
        # gt_semantic_heat /= 65535.0
        ...
        if self.reduce_zero_label:
            # avoid using underflow conversion
            gt_semantic_heat[gt_semantic_heat == 0] = 255
            gt_semantic_heat = gt_semantic_heat - 1
            gt_semantic_heat[gt_semantic_heat == 254] = 255
        # modify if custom classes
        if results.get('label_map', None) is not None:
            # Add deep copy to solve bug of repeatedly
            # replace `gt_semantic_seg`, which is reported in
            # https://github.com/open-mmlab/mmsegmentation/pull/1445/
            gt_semantic_heat_copy = gt_semantic_heat.copy()
            for old_id, new_id in results['label_map'].items():
                gt_semantic_heat[gt_semantic_heat_copy == old_id] = new_id
        results['gt_heat_map'] = gt_semantic_heat
        results['seg_fields'].append('gt_heat_map')
