...
class PANDecoder(BaseDecodeHead):
    def __init__(self, encoder_channels, decoder_channels, upscale_mode: str = "bilinear", 
                 upsampling=4,**kwargs):
        super(PANDecoder, self).__init__(**kwargs)
    ...
    def forward(self, *features):
        bottleneck = features[0][-1]
        x5 = self.fpa(bottleneck)  # 1/32
        x4 = self.gau3(features[0][-2], x5)  # 1/16
        x3 = self.gau2(features[0][-3], x4)  # 1/8
        x2 = self.gau1(features[0][-4], x3)  # 1/4
        x2 = self.segmentation_head(x2)
        return x2
