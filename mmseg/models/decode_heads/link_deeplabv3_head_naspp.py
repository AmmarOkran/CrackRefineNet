...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, separable=False):
        ...
        modules.append(ASPPConvModule(in_channels, out_channels, rate5))
        modules.append(ASPPPooling(in_channels, out_channels))

        self.convs = nn.ModuleList(modules)

        self.project = nn.Sequential(
            nn.Conv2d(7 * out_channels, out_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(),
            nn.Dropout(0.5),
        )
...
@HEADS.register_module()
class DeepLabV3PlusLinkHeadNAspp(BaseDecodeHead):
    def __init__(
        self,
        encoder_channels,
        prefinal_channels=32,
        ...
        upsampling=8,
        atrous_rates=(3, 6, 12, 24, 36),
        # atrous_rates=(12, 24, 36),  1280 1536
        output_stride=16,
        **kwargs
    ):
        super().__init__(**kwargs)
