...
@HEADS.register_module()
class DeepLabV3PlusLinkAttHeadUpdated(BaseDecodeHead):
    def __init__(
        self,
        encoder_channels,
        prefinal_channels=32,
        ...
        **kwargs
        ...
        self.Att = [AttentionBlock(F_g=encoder_channels[ch], 
                                   F_l=encoder_channels[ch], 
                                    n_coefficients=encoder_channels[ch]//2).cuda()
                     for ch in reversed(range(len(encoder_channels[:-1])))]
        
        self.upconv = nn.ModuleList()
        for i in reversed(range(len(encoder_channels))):
            if i != 0:
                self.upconv.append(ConvBlock(encoder_channels[i], encoder_channels[i-1]))
    ...
    def forward(self, *features):
        ...
        for i, m in enumerate(self.rep_co):
            ...
            # rp_conv.append(self.up_feat[i](m(aspp_features_1)))
        ...
        for i, decoder_block in enumerate(self.link.blocks):
            skip = skips[i] if i < len(skips) else None
            x = decoder_block(x, skip)
            # s4 = self.Att[i](gate=x, skip_connection=skip)
            # x = torch.cat([x, s4], dim=1)
            # x = self.upconv[i](x) + skip
