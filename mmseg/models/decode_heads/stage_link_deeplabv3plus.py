...
class TransposeX2(nn.Sequential):
    def __init__(self, in_channels, out_channels, stride=2, padding=1, use_batchnorm=True):
        super().__init__()
        layers = [
            nn.ConvTranspose2d(in_channels, out_channels, kernel_size=4, stride=stride, padding=padding),
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=2, padding=1, use_batchnorm=True):
        ...
        self.block = nn.Sequential(
            ...
            TransposeX2(in_channels // 4, in_channels // 4, stride=stride, padding=padding, use_batchnorm=use_batchnorm),
            md.Conv2dReLU(
                in_channels // 4,
                out_channels,
                kernel_size=1,
...
class LinknetDecoder(BaseDecodeHead):
    def __init__(
        ...
        **kwargs
        ...
        encoder_channels = encoder_channels[::-1]
        stride_values = [2, 2, 2, 4]  # Replace with your actual stride values
        padding_values = [1, 1, 1, 0]  # Replace with your actual padding values
        channels = list(encoder_channels) + [prefinal_channels]

        self.blocks = nn.ModuleList(
            [DecoderBlock(channels[i], channels[i + 1], stride=stride_values[i], padding=padding_values[i], use_batchnorm=use_batchnorm) for i in range(n_blocks)]
        )
...
@HEADS.register_module()
class NStageDeepLabV3PlusLinkHead(BaseDecodeHead):
    def __init__(
        self,
        encoder_channels,
        prefinal_channels=32,
        ...
        **kwargs
        ...
        self.block3 = nn.Sequential(
            SeparableConv2d(
                highres_out_channels + out_decode + 32,
                out_decode//8,
                kernel_size=3,
                padding=1,
                bias=False,
            ),
            nn.BatchNorm2d(out_decode//8),
            nn.ReLU(),
        )
    ...
    def forward(self, *features):
        ...
        high_res_features = self.deeplab.block1(skips[-2])
        h_res_feats = self.block0(skips[-1])
        concat_features1 = torch.cat([aspp_features_2, high_res_features], dim=1)
        concat_features2 = torch.cat([h_res_feats, self.f_up(concat_features1)], dim=1)
        fused_features1 = self.deeplab.block2(concat_features1)
        fused_features2 = self.block3(concat_features2)

        # LinkNet
        skips[2] = fused_features1 + skips[2]
        skips[3] = fused_features2 + skips[3]
        x = torch.cat([x, aspp_features_1], dim=1)
        for i, decoder_block in enumerate(self.link.blocks):
            skip = skips[i] if i < len(skips) else None
            x = decoder_block(x, skip)

        x1 = self.b_fuse(fused_features2)
        x = x + x1
        x = self.dropout(x)
