...
class LinknetDecoder(BaseDecodeHead):
    def __init__(
        ...
        **kwargs
        ...
        # encoder_channels = encoder_channels[::-1]
        encoder_channels = [1280, 3072, 1536, 768]
        decoder_channels = [1024, 512, 256] + [prefinal_channels]

        channels = list(encoder_channels) + [prefinal_channels]

        # self.blocks = nn.ModuleList(
        #     [DecoderBlock(channels[i], channels[i + 1] if i != n_blocks-1 else channels[i + 1], use_batchnorm=use_batchnorm) for i in range(n_blocks)]
        # )
        self.blocks = nn.ModuleList(
            [DecoderBlock(encoder_channels[i], decoder_channels[i], use_batchnorm=use_batchnorm) for i in range(n_blocks)]
        )
...
@HEADS.register_module()
class DeepLabV3PlusLinkHeadConcat(BaseDecodeHead):
    def __init__(
        self,
        encoder_channels,
        prefinal_channels=32,
        ...
        **kwargs
        ...
        self.conv_feat = [nn.Conv2d(ch, 32, 1).cuda() for ch in [512, 256, 128]]
    ...
    def forward(self, *features):
        ...
        for i, m in enumerate(self.rep_co):
            ...
            rp_conv.append(self.up_feat[i](m(aspp_features_1)) + skips[i])

        ## LinkNet
        # repeated_tensor = torch.cat([self.up2(aspp_features_1), self.up2(aspp_features_1)], dim=1)
        # skips[0] = repeated_tensor + skips[0]
        # skips[1] = self.up4(aspp_features_1) + skips[1]
        # skips[2] = fused_features + skips[2]
        x = torch.cat([x, aspp_features_1], dim=1)
        link_res = []
        for i, decoder_block in enumerate(self.link.blocks):
            skip = skips[i] if i < len(skips) else None
            skip = torch.cat([skip, rp_conv[i]], dim=1) if i < len(skips) else None
            x_copy, x = decoder_block(x, skip) ####
            if skip is not None:
                x = torch.cat([x_copy, x, skip], 1)

        x1 = self.up2(self.b_fuse(fused_features))
        # weight = self.sigmoid(torch.sum(torch.stack(link_res), dim=0))
        # x_w = x * weight
        # x = x + x_w
        x = x + x1
        x = self.dropout(x)
