...
class DeepLabV3HeadSweak(BaseDecodeHead):
    def __init__(self, 
                 out_channels=256, 
        ...
        super(DeepLabV3HeadSweak, self).__init__(**kwargs)
...
@HEADS.register_module()
class DeepLabV3PlusHeadSweak(BaseDecodeHead):
    def __init__(
        self,
        encoder_channels,
        out_decode=256,
        ...
        **kwargs
        ...
        # self.outc1 = (OutConv(256, self.out_channels))
        # self.outc2 = (OutConv(out_decode, self.out_channels))
        # self.outc3 = (OutConv(128 // factor, 1))
        # self.outc4 = (OutConv(64, 1))
        # self.upsampling = nn.UpsamplingBilinear2d(scale_factor=upsampling) if upsampling > 1 else nn.Identity()
        self.segmentation_head1 = SegmentationHead(
            in_channels=out_decode,
            out_channels=self.out_channels,
            kernel_size=1,
            upsampling=upsampling*8,
        )
        self.segmentation_head = SegmentationHead(
            in_channels=out_decode,
            out_channels=self.out_channels,
    ...
    def forward(self, *features, bg=False):
        dec_out = []
        logits_list = []
        aspp_features = self.aspp(features[0][-1])
        dec_out.append(aspp_features if bg else None)
        logits_list.append(self.segmentation_head1(aspp_features) if bg else None)
        # dec_out.append(self.outc1(aspp_features) if bg else None)
        # aspp_features2 = self.aspp2(features[0][-1])
        # aspp_features = self.att(aspp_features, features[0][-2])
        aspp_features = self.up(aspp_features)
        # dec_out.append(aspp_features)
        ...
        # logits = self.outc2(fused_features)
        # logits = self.outc2(fused_features)
        dec_out.append(fused_features if bg else None)
        # dec_out.append(logits if bg else None)
        logits = self.segmentation_head(fused_features)
        logits_list.append(logits if bg else None)
        # z = self.up2(self.block3(features[0][0]))
        if bg:
            return dec_out, logits_list
        else:
            return logits
