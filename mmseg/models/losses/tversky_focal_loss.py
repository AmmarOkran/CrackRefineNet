...
def tversky_focal_loss(pred,
    ...
    for i in range(num_classes):
        if i != ignore_index:
            tversky_loss = binary_tversky_focal_loss(
                pred[:, i],
                target[..., i],
...
@weighted_loss
def binary_tversky_focal_loss(pred,
                        target,
                        valid_mask,
                        alpha=0.3,
                        beta=0.7,
...
class TverskyFocalLoss(nn.Module):
    ...
    def forward(self, pred, target, **kwargs):
        ...
        loss = self.loss_weight * torch.pow(tversky_focal_loss(
            pred,
            one_hot_target,
            valid_mask=valid_mask,
            alpha=self.alpha,
            ...
            ignore_index=self.ignore_index), self.gamma)
        return loss
