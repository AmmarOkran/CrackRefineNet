...
class TransMUNet(BaseModule):


    def __init__(self, depth = 4, dims = [144, 240, 320]):
        super().__init__()
    ...
    def forward(self, x):
        ...
        B_out  = self.boundary(features[1])
        features[1] = features[1] + B_out.repeat_interleave(int(features[1].shape[1]), dim=1)
        
        return features, B_out   