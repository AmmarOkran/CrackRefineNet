...
class ModNLBlockND(nn.Module):
    def __init__(self, in_channels, inter_channels=None, mode='embedded', 
                 dimension=3, bn_layer=True, dropout_prob = 0.5, reduction=16):
        super(ModNLBlockND, self).__init__()
    ...
    def forward(self, x):
        ...
        attention_ll1 = self.SE_ll(x)
        attention_ll2 = self.SE_ll(x)
        attention_ll3 = self.SE_ll(x)
        att2 = attention_ll1 * attention_ll2
        att2 = self.SE_ll(att2)
        att_weight = self.sigmoid(att2)

        # x = x * attention_ll2
        att3 = attention_ll3 * att_weight
        z = x + att3
        return z
